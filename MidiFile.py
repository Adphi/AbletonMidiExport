...
class GenericEvent(object):
    ...
    def __eq__(self, other):
        ...
        if self.type == 'pitchWheelEvent':
            return False

        if self.type == 'channelPressureEvent':
            return False
...
class ChannelPressureEvent(GenericEvent):
    '''
    A class that encapsulates a channel pressure event.
    '''

    def __init__(self, channel, time, vibrato, ordinal=1, insertion_order=0):
        self.chanel = channel
        self.vibrato_value = vibrato
        super(ChannelPressureEvent, self).__init__('channelPressureEvent', time, ordinal, insertion_order)
...
class MIDITrack(object):
    ...
    def addChannelPressureEvent(self, channel, time, vibrato_value, insertion_order=0):
        '''
        Add a channel pressure event.
        '''
        self.eventList.append(ChannelPressureEvent(channel, time, vibrato_value, insertion_order=insertion_order))
    ...
    def processEventList(self):
        ...
        for thing in self.eventList:
            ...
            elif thing.type == 'channelPressureEvent':
                event = MIDIEvent('ChannelPressureEvent', thing.time * TICKSPERBEAT, thing.ord, thing.insertion_order)
                event.vibrato_value = thing.vibrato_value
                event.channel = thing.channel
                self.MIDIEventList.append(event)
    ...
    def writeEventsToStream(self):
        ...
        for event in self.MIDIEventList:
            ...
            elif event.type == 'ChannelPressureEvent':
                code = 0xD << 4 | event.channel
                varTime = writeVarLength(event.time)
                for timeByte in varTime:
                    self.MIDIdata = self.MIDIdata + struct.pack('>B',timeByte)
                self.MIDIdata = self.MIDIdata + struct.pack('>B',code)
                self.MIDIdata = self.MIDIdata + struct.pack('>B',event.vibrato_value)
...
class MIDIFile(object):
    ...
    def addTempo(self, track: object, time: object, tempo: object) -> object:
        """
    ...
    def addChannelPressureEvent(self, track, channel, time, vibrato):
        '''
        ...
        :param channel: the MIDI channel to assign to the event. [Integer, 0-15]
        :param time: The time (in beats) at which the event is placed [Float].
        :param vibrato: [Integer 0-127]
        '''
        if self.header.numeric_format == 1:
            track = track + 1
        self.tracks[track].addChannelPressureEvent(channel, time, vibrato, insertion_order=self.event_counter)
        self.event_counter = self.event_counter + 1
